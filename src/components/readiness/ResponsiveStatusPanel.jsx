// AP3X — ResponsiveStatusPanel — Run 9
import { statusColor } from "../../utils/readinessScoringUtils.js";
import { statusIcon }  from "../../utils/productTestUtils.js";

export default function ResponsiveStatusPanel({ checks }) {
  if (!checks || checks.length === 0) return <div className="panel" style={{ color:"var(--muted)", padding:20 }}>Run the product test to see responsive status.</div>;
  return (
    <div className="panel">
      <div className="section-label" style={{ marginBottom:10 }}>Responsive Layout Status</div>
      {checks.map((c) => (
